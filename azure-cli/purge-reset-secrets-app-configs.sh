...
if [ -n "$OLD_KEYS" ]; then
  ...
  az functionapp config appsettings delete --name "$DIRECT_FUNCTION_NAME" --resource-group "$RESOURCE_GROUP" --setting-names $OLD_KEYS
  az functionapp config appsettings delete --name "$SINK_FUNCTION_NAME" --resource-group "$RESOURCE_GROUP" --setting-names $OLD_KEYS
...
for k in $(jq -r 'to_entries|map("\(.key)=\(.value|tostring)")|.[]' azure-app-settings.json | grep 'json'); do
  ...
  az keyvault secret set --vault-name "$KEY_VAULT" --name "$CONFIG_NAME" --file "$CONFIG_VALUE"
done
...
for k in $(az keyvault secret list --vault-name "$KEY_VAULT" | jq -r '.[].name'); do
  ...
  az functionapp config appsettings set --name "$DIRECT_FUNCTION_NAME" --resource-group "$RESOURCE_GROUP" --settings "${k}=@Microsoft.KeyVault(SecretUri=https://confluent-cloud-keyvault.vault.azure.net/secrets/${k})"
  echo "Now in ${SINK_FUNCTION_NAME}"
  az functionapp config appsettings set --name "$SINK_FUNCTION_NAME" --resource-group "$RESOURCE_GROUP" --settings "${k}=@Microsoft.KeyVault(SecretUri=https://confluent-cloud-keyvault.vault.azure.net/secrets/${k})";
