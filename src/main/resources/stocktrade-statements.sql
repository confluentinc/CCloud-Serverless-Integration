...
CREATE STREAM USER_TRADES WITH (kafka_topic = 'user_trades', value_format = 'PROTOBUF' ) AS SELECT s.userid as USERID,u.regionid,quantity,symbol,price,account,side FROM STOCKTRADE s LEFT JOIN users u on s.USERID = u.userid;
